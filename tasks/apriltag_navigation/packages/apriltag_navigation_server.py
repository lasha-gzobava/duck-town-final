import sys
import os
import signal
import threading
import socket

script_dir   = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.join(script_dir, '..', '..', '..')
sys.path.insert(0, project_root)

import cv2
import yaml
from flask import Flask, Response, render_template_string, request, jsonify

from tasks.apriltag_navigation.packages.agent import TrafficNavigationAgent
from servers.apriltag_navigation.visualization import create_traffic_nav_visualization
from servers.templates.apriltag_navigation import APRILTAG_NAVIGATION_TEMPLATE as HTML_TEMPLATE

from duckiebot.camera_driver import CameraDriver
from duckiebot.wheel_driver import DaguWheelsDriver
from duckiebot.wheel_driver.wheels_driver_abs import WheelPWMConfiguration
from launcher.ports import find_available_port
from servers.common import make_frame_generator, shutdown_cleanup, suppress_http_logs

LANE_CONFIG_FILE     = os.path.join(project_root, 'config', 'lane_servoing_config.yaml')
LANE_HSV_CONFIG_FILE = os.path.join(project_root, 'config', 'lane_servoing_hsv_config.yaml')


def _get_student_module():
    from tasks.visual_lane_servoing.packages import visual_servoing_activity
    return visual_servoing_activity


app = Flask(__name__)

camera     = None
wheels     = None
agent      = None
running    = False
stop_event = threading.Event()


def visualize(frame_bgr):
    global running
    if agent is None or wheels is None:
        return frame_bgr

    frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
    pwm_left, pwm_right = agent.compute_commands(frame_rgb)

    if running:
        wheels.set_wheels_speed(pwm_left, pwm_right)
    else:
        wheels.set_wheels_speed(0.0, 0.0)

    debug_info = agent.last_debug_info
    return create_traffic_nav_visualization(frame_bgr, debug_info, pwm_left, pwm_right)


generate_frames = make_frame_generator(lambda: camera, visualize, quality=50, rgb=False)


@app.route('/')
def index():
    return render_template_string(HTML_TEMPLATE, config=agent.lane_agent if agent else None,
                                  hostname=socket.gethostname(), virtual=False)


@app.route('/video')
def video():
    return Response(generate_frames(), mimetype='multipart/x-mixed-replace; boundary=frame')


@app.route('/start', methods=['POST'])
def start():
    global running
    running = True
    print("[Control] Started")
    return jsonify({'status': 'running'})


@app.route('/stop', methods=['POST'])
def stop():
    global running
    running = False
    if wheels:
        wheels.set_wheels_speed(0.0, 0.0)
    print("[Control] Stopped")
    return jsonify({'status': 'stopped'})


@app.route('/running')
def get_running():
    return jsonify({'running': running})


@app.route('/update_config', methods=['POST'])
def update_config():
    data = request.json or {}
    la = agent.lane_agent
    la.p_gain     = float(data.get('k_d',   la.p_gain))
    la.d_gain     = float(data.get('k_phi', la.d_gain))
    la.base_speed = float(data.get('const', la.base_speed))
    try:
        with open(LANE_CONFIG_FILE, 'r') as f:
            saved = yaml.safe_load(f) or {}
        saved['p_gain']     = la.p_gain
        saved['d_gain']     = la.d_gain
        saved['base_speed'] = la.base_speed
        with open(LANE_CONFIG_FILE, 'w') as f:
            yaml.dump(saved, f, default_flow_style=False)
    except Exception as e:
        print(f"[AprilTagNav] Could not save config: {e}")
    return jsonify({'status': 'ok'})


@app.route('/get_hsv')
def get_hsv():
    return jsonify(_get_student_module().get_hsv_bounds())


@app.route('/update_hsv', methods=['POST'])
def update_hsv():
    data = request.json or {}
    mod  = _get_student_module()
    current = mod.get_hsv_bounds()
    current.update({k: int(v) for k, v in data.items()})
    mod.set_hsv_bounds(
        [current['yellow_lower_h'], current['yellow_lower_s'], current['yellow_lower_v']],
        [current['yellow_upper_h'], current['yellow_upper_s'], current['yellow_upper_v']],
        [current['white_lower_h'],  current['white_lower_s'],  current['white_lower_v']],
        [current['white_upper_h'],  current['white_upper_s'],  current['white_upper_v']],
    )
    try:
        with open(LANE_HSV_CONFIG_FILE, 'w') as f:
            yaml.dump(current, f, default_flow_style=False)
    except Exception as e:
        print(f"[AprilTagNav] Could not save HSV config: {e}")
    return jsonify({'status': 'ok'})


@app.route('/status')
def status():
    if agent is None:
        return jsonify({'status': 'not_initialized'})
    la = agent.lane_agent
    return jsonify({
        'status': 'active',
        'frame_count': agent.frame_count,
        'config': {
            'p_gain': la.p_gain, 'd_gain': la.d_gain,
            'base_speed': la.base_speed, 'detection_threshold': la.detection_threshold,
        },
        'state': agent.state,
        'state_remaining': round(agent.last_debug_info.get('state_remaining', 0.0), 2),
        'detected_signs': [
            {'tag_id': s['tag_id'], 'sign_type': s['sign_type'], 'area': int(s['area'])}
            for s in agent.last_debug_info.get('detected_signs', [])
        ],
        'event_log': list(agent.event_log),
    })


def main():
    global camera, wheels, agent

    import argparse
    ap = argparse.ArgumentParser(description="AprilTag Traffic-Sign Navigation — Real Robot")
    ap.add_argument('--port', type=int, default=5000)
    args = ap.parse_args()

    suppress_http_logs()
    print('=' * 60)
    print('APRILTAG TRAFFIC-SIGN NAVIGATION — REAL ROBOT')
    print('=' * 60)

    def _init_wheels():
        global wheels
        wheels = DaguWheelsDriver(WheelPWMConfiguration(), WheelPWMConfiguration())
        print('[Init] Wheels ready')

    def _init_camera():
        global camera
        cam = CameraDriver()
        cam.start()
        camera = cam
        print('[Init] Camera ready')

    def _init_agent():
        global agent
        agent = TrafficNavigationAgent()
        la = agent.lane_agent
        print(f'[Init] Agent ready  p={la.p_gain} d={la.d_gain} spd={la.base_speed}')

    threading.Thread(target=_init_wheels, daemon=True).start()
    threading.Thread(target=_init_camera, daemon=True).start()
    threading.Thread(target=_init_agent,  daemon=True).start()

    def _shutdown(signum, frame):
        shutdown_cleanup(wheels, camera, stop_event)
        sys.exit(0)

    signal.signal(signal.SIGTERM, _shutdown)
    signal.signal(signal.SIGINT,  _shutdown)

    web_port = find_available_port(args.port)
    print(f'\nWeb Interface: http://{socket.gethostname()}.local:{web_port}')
    print('=' * 60 + '\n')

    try:
        app.run(host='0.0.0.0', port=web_port, debug=False, threaded=True)
    except (KeyboardInterrupt, SystemExit):
        pass
    finally:
        shutdown_cleanup(wheels, camera, stop_event)


if __name__ == '__main__':
    sys.exit(main())
