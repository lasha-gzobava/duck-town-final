...
def visualize(frame):
    ...
    return create_traffic_nav_visualization(bgr, debug_info, pwm_left, pwm_right)
...
@app.route('/')
def index():
    return render_template_string(HTML_TEMPLATE, config=agent.lane_agent, hostname=socket.gethostname())
...
def reset():
    ...
    if wheels is not None and agent is not None:
        spd = agent.lane_agent.base_speed
...
def status():
    ...
    return jsonify({
        ...
        'config': {'p_gain': la.p_gain, 'd_gain': la.d_gain,
...
def main():
    ...
    import argparse
    ap = argparse.ArgumentParser(description="AprilTag Traffic-Sign Navigation Server")
    ...
    suppress_http_logs()
    print("=" * 60)
    print("APRILTAG TRAFFIC-SIGN NAVIGATION SERVER")
    print("=" * 60)
    ...
    la = agent.lane_agent
    print(f"  p_gain={la.p_gain}, d_gain={la.d_gain}, base_speed={la.base_speed}")
